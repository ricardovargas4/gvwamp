...
class ResponsavelController extends Controller
{
    ...
    public function filtro(ResponsavelRequest $request,$filtroId_processo=null,$filtroUsuario=null){
        ...
        if($nivel>2){
            $filtroUsuContr = json_decode(Auth::user()->id);
        }else{            
        ...
        $processos = Processo::orderBy('nome')->get();
            ...
            return view('responsavel.listagem',compact('resp','users','processos','filtroId_processo','filtroUsuario','filtro','teste','nivel'));
        }
    ...
    public function remove(ResponsavelRequest $request){
      ...
      //  return redirect()->action('ResponsavelController@lista',['page'=>$page]);
        ...
        $data=['filtroId_processo' =>$request->filtroId_processo ,
               'filtroUsuario' =>  $request->filtroUsuario,
               'page' => $request->page];
        ...
        if(isset($request->id_processo)){
            if(Processo::find($request->id_processo)->tipo==3){
                ...
                if(!$resp->isEmpty()){
                    ...
                    Responsavel::whereId($id)->update($request->except('_token','filtroId_processo','filtroUsuario','page'));
                }
            }else{
                Responsavel::whereId($id)->update($request->except('_token','filtroId_processo','filtroUsuario','page'));
            }
    ...
    public function direcionar(ResponsavelRequest $request){
        $id = $request->id;
        if(isset($request->id_processo)){
            Responsavel::whereId($id)->update($request->except('_token','filtroId_processo','filtroUsuario','page'));
        }
    ...
    public function adiciona(ResponsavelRequest $request){
        ...
        if(Processo::find($request->id_processo)->tipo==3){
            ...
            if(!$resp->isEmpty()){
                ...
                Responsavel::create($request->except('_token','filtroId_processo','filtroUsuario','page'));
            }
        }else{
            Responsavel::create($request->except('_token','filtroId_processo','filtroUsuario','page'));
        }
